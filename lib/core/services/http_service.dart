...
/// Servicio base para peticiones HTTP en LogiScan
class HttpService {
  ...
  String _sanitizeBodyForLog(String url, String body) {
    // En LogiScan de momento no necesitamos lógica especial;
    // dejamos la implementación sencilla.
    return body;
  }
  ...
  Future<ApiResponse<T>> get<T>(
    ...
    try {
      ...
      final response =
          await client.get(uri, headers: _headers).timeout(const Duration(seconds: 30));

      AppLogger.apiCall(uri.toString(), method: 'GET');
      final sanitizedBody = _sanitizeBodyForLog(uri.toString(), response.body);
      AppLogger.apiResponse(uri.toString(),
          statusCode: response.statusCode, body: sanitizedBody);
